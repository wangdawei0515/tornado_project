...
class Permission(Base):
    ...
    #roles = relationship("Role", secondary="permission_to_role")
...
class Role(Base):
    ...
    name = Column(String(50), nullable=False)
    #角色表和用户表多对多查询关系
    users = relationship("User", secondary="permission_user_to_role")
    # users = relationship("User", secondary=UserToRole.__table__)
    #角色表和权限表多对多查询关系
    permissions = relationship("Permission", secondary="permission_to_role")
